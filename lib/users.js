...
exports.findBan = function (ip) {
    return _.find(bans, function (ban) {
        return ban.ip == ip;
    });
};
...
exports.login = function (session, name, password, id, callback) {
    ...
    model.User.find({name: name}, 1, function (err, result) {
        ...
        if (!id && result.length) {
            var row = result[0];
            if (!password || crypto.pbkdf2Sync(password, row.password_salt, 10000, 256, "sha1").toString('base64') != row.password) {
        ...
        log.trace('Beginning login for ' + name + ' with id ' + id);
        if (id) {
            doLogin(id);
        } else {
            const gotResults = (err, nextval) => {
                if (err) {
            ...
            switch (model.db.driver_name) {
                case "mysql":
                    let dbName = model.db.driver.config.database;
                    model.db.driver.execQuery(`SELECT \`AUTO_INCREMENT\` as \`nextval\` FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = "${dbName}" AND TABLE_NAME = "user";`, (err, nextval) => {
                        model.db.driver.execQuery(`ALTER TABLE \`user\` AUTO_INCREMENT = ${nextval[0].nextval + 1};`, () => gotResults(err, nextval));
                    });
                    break;
                default:
                    model.db.driver.execQuery('SELECT nextval(\'object_id_seq\');', gotResults);
                    break;
            }
        }
    });
...
exports.logout = function (id, session, reason) {
    ...
    log.debug('Logged out ' + user.name + '/' + user.id + ' (' + reason + ')');
};
