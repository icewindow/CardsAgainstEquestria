...
var Game = function (host) {
    ...
    this.redrawHandLimit = 1;
    this.redrawHandEnabled = true;
    ...
    this.redrawPlayerHand = function(user) {
        let player = _.find(this.players, p => user.id === p.user.id);
        if (!player) {
            return;
        }
        if (!this.redrawHandEnabled) {
            trace(this, formatPlayer(player) + " attempted to redraw hand when hand redrawing is disabled");
            return;
        }
        if (this.redrawHandLimit < 0 || player.handsRedrawn < this.redrawHandLimit) {
            trace(this, "Dealing new hand to " + formatPlayer(player));
            player.hand = [];
            player.handsRedrawn++;
            this.dealHand(player);
            this.sendUpdates();
        } else {
            trace(this, formatPlayer(player) + " exceeded redraw limit");
        }
    };
    ...
    this.update = function (data) {
        ...
        var redrawHandLimit = parseInt(data.redrawHandLimit);
        this.redrawHandLimit = redrawHandLimit;
        this.redrawHandEnabled = redrawHandLimit !== 0
    ...
    this.roundState = function () {
        ...
        let redraws = {};
        _.each(this.players, p => redraws[p.user.id] = p.handsRedrawn);
        ...
        return {round: this.round, czar: this.czar.user.id, points: points, redraws: redraws, timeLeft: timeLeft};
    };
    ...
    this.pushHand = function (player) {
        var hand = {
            ...
            handsRedrawn: player.handsRedrawn
        };
    ...
    this.toJsonFormat = function (includePassword) {
        var data = {
            ...
            redrawHandLimit: this.redrawHandLimit,
            redrawHandEnabled: this.redrawHandEnabled,
